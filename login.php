...
//    $LOGIN_PAGE_DOMAIN_AND_PATH = "Set this if login landing page != this page";
    ...
    $nameshares = new Bitcoin($RPC_SERVER_USER, $RPC_SERVER_PASS, $RPC_SERVER_ADDRESS, $RPC_SERVER_PORT, $RPC_SERVER_PATH);

    $nameshares->open($RPC_SERVER_WALLET);
    $nameshares->unlock(2, $RPC_SERVER_WALLET_PASS);   
    
    if(isset($_REQUEST["client_key"]))
    {
        $loginPackage = $nameshares->wallet_login_finish($_REQUEST["server_key"], $_REQUEST["client_key"], $_REQUEST["signed_secret"]);
        ...
        $userAccount = $nameshares->blockchain_get_account($loginPackage["user_account_key"]);
        ...
        $url = $nameshares->wallet_login_start($NAMESHARES_USER_NAME);
        $url .= $absolute_url;
        ?>
